...
function postRequest(url, csrftoken, login, password, enc_pass, headers){
    const postCurl = new Curl();
 
    postCurl.setOpt('URL', 'https://www.instagram.com' + url);
    postCurl.setOpt('FOLLOWLOCATION', true);
    postCurl.setOpt('COOKIEFILE', './cookie/test');
    postCurl.setOpt('COOKIEJAR', './cookie/test');
...
async function login(login, pass){
    ...
    let pubKey;
    let pubKeyId;
    await getrequest('/accounts/login').then((arr) => {
    ...
    let encPassData = encrypt({password: pass, publicKey: pubKey, publicKeyId: pubKeyId});
